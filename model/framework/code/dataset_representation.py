...
# This file has been downloaded from https://github.com/rolayoalarcon/MolE/tree/main and adapted
# to fit Ersilia
...
def load_pretrained_model(pretrain_architecture, pretrained_model, pretrained_dir = "./ckpt", device="cuda:0"):
    ...
    config = yaml.load(open(os.path.join(root, "..", "..", "checkpoints/config.yaml"), "r"), Loader=yaml.FullLoader)
    ...
    model_pth_path = os.path.join(root, "..", "..", "checkpoints/model.pth")
...
def process_dataset(dataset_path, 
    ...
    if pretrained_model == "ECFP4":
        ...
        pmodel = load_pretrained_model(pretrain_architecture, pretrained_model, device="cpu")
        # Obtain the requested representation
        udl_representation = batch_representation(smiles_df, pmodel, device="cpu")
